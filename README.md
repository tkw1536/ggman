...
Note: In case you don't trust a Makefile, you can also build using standard go tools.
The Makefile does nothing else except automatically include the current version number, and pass appropriate arguments to make the build reproducible. 
...
`ggman parse` also understands forge tree URLs and can print branch and path information.
...
However, three characters are treated differently:
- `^` is replaced by the first unused component of the URI (typically a hostname)
- `%` is replaced by the second unused component of the URI (commonly a username)
- `!` consumes the first component of the URI without using it in the output.
   Use this to e.g. replace the hostname with a different one.
...
- `ggman comps` behaves exactly like `ggman parse --comps`
