...
ggcd () {
	REPO="$(ggman --for "$1" ls --exit-code --one 2>&1)" && echo "$REPO" && cd "$REPO"
}

# ggshow is like ggcd, except that is displays the 
ggshow () {
	REPO="$(ggman --for "$1" ls --exit-code --one 2>&1)" && ggman --for "$REPO" show --no-patch 2>&1
}

# ggcode is like ggcd, except it opens an editor (here vscode) instead of cding. 
ggcode () {
	REPO="$(ggman --for "$1" ls --exit-code --one 2>&1)" && echo "$REPO" && code "$REPO"
}
